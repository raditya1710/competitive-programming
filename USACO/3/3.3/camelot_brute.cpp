...
int main(){
  ...
  for(int i = 0;i < r; ++i){
    for(int j = 0;j < c; ++j){
      ...
      for(int k = 0;k < r; ++k){
        for(int l = 0;l < c; ++l){
          min_dist = min(min_dist, dist_total + manhattan(mp(i,j),mp(k,l)));
        }
      }
        ...
        for(int l = 0;l < r; ++l){
          for(int m = 0;m < c; ++m){
            if(dist[knights[k].fi][knights[k].se][l][m] != -1 && dist[l][m][i][j] != -1){
              min_dist = min(min_dist, dist_now + dist[knights[k].fi][knights[k].se][l][m] + dist[l][m][i][j] + manhattan(mp(i,j), mp(l,m)));
            }
          }
        }
      }
